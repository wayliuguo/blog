...
**前置模型**：栈存值类型/作用域/引用地址、函数结束自动弹出；堆存对象本体、生命周期比函数作用域长、交给 GC——这正是"需要 GC 而非靠栈弹出"的原因。

**GC 判定策略的演进链**（链条每一环都是上一环缺陷的解法，按知识文档演进脉络）：
- **引用计数**（最早朴素思路）：每对象记引用次数、归零即回收 → 致命缺陷**无法处理循环引用**（互指孤岛永不回收）→ 引出"以根为准"的判定
- **标记清除**（击败引用计数成主流）：从根（全局对象/执行栈）遍历判可达、未标记即清，能处理循环引用 → 代价**内存地址不连续、产生碎片** → 引出标记整理
- **标记整理**：把存活对象搬移到连续区域消碎片；V8 老生代的"标记清除 + 标记整理"就是它消除碎片后的实际形态

**策略如何落地 → V8 堆结构**（分代收集思想）：
- **分代规律**：绝大多数对象生命周期短 → 分代收集：年轻对象高频回收、长命对象低频回收
- **新生代 Scavenge**：半空间复制（From/To 互换），存活对象复制到 To、整体清空 From；牺牲一半空间换极高吞吐
- **晋升**：多次存活 / To 空间放不下 → 晋升老生代
- **老生代**：标记清除 + 标记整理（存活率高、不适合复制、低频回收）

**降低主线程停顿**：一次全量标记扫描会 Stop-The-World 卡顿 → 增量标记（拆成小步穿插执行间隙）、并发标记（后台线程）、惰性/并发清理（清除也挪后台）——思想是"拆小、挪后台"；老生代实际是"标记→清除→整理"三段式，Free List 优先填碎片、按需才压缩。

**五类泄漏**：意外全局变量（`use strict` 拦截）、闭包固化大对象（最小捕获+置空）、事件监听未移除（removeEventListener/AbortController）、定时器未清理、分离 DOM（置空引用）——本质都是**本应回收却因残留强引用无法释放**。

**检测**：DevTools Memory 三招——Performance 看曲线（台阶式/只升不降=泄漏）、Heap Snapshot 按 Retained Size 找大对象 + 看 Retainers 引用路径、分配采样找持续分配者。

**WeakMap/WeakSet**：弱引用不阻止回收、键必须为对象、不可迭代——缓存/私有数据用它防强引用持有型泄漏。
