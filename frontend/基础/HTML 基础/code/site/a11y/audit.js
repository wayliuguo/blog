...
function report(name, root, focusIds) {
    ...
    log('  标题层级：全页共 ' + heads.total + ' 个标题，跳级 ' + heads.skips.length + ' 处' + (heads.skips.length ? ' → ' + heads.skips.join('、') : ''))
    for (const [label, id] of focusIds) {
        const f = checkFocus(id)
        log(
            '  键盘可达（' + label + '）：focus() 后 activeElement ' +
                (f.focused ? '就是它自己 → 可聚焦' : '仍是 body → 不可聚焦') +
                '，tabIndex = ' + f.tabIndex + '，聚焦时 outline-style = ' + f.outline
        )
    }
    const fails = contrasts.filter((c) => !c.pass)
    const lowest = contrasts.reduce((min, c) => Math.min(min, c.ratio), Infinity)
    log(
        '  对比度：检查 ' + contrasts.length + ' 处文本，不达标 ' + fails.length + ' 处（最低 ' + lowest + ':1，正文要求 ≥ 4.5:1）' +
