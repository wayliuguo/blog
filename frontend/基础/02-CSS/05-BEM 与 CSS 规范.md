...
- 当项目已经引入 Tailwind 这类原子化方案时，通常就不再需要 BEM，二者是不同路线的取舍。
...
- BEM 与 CSS 规范
  ...
  - BEM 原理
    ...
    - `__` 连接元素 / `--` 连接修饰符
    - 不写父级链中间元素（`card__title` 非 `card__header__title`）
    - 修饰符挂基础类使用，做增量覆盖
    - 块判断：能否脱离上下文独立复用
  - 代码应用
    - 小写 + 连字符命名
    - 语义优先：`card--primary` 而非 `card--red`
    - Sass 嵌套：`&__` / `&--`
    - 与 JS 解耦：`data-*` / `js-` 钩子
    - 不适用的场景：一次性样式 / Tailwind 项目
  - CSS 工程化对比
    - 普通全局 CSS：零成本但易冲突
    - CSS Module：哈希类名局部作用域
    - CSS-in-JS：styled-components / Emotion
    - Tailwind 原子化：JIT 按需生成
  - 选型考虑
    - 项目规模 / 团队熟悉度 / 构建链
    - 可维护性 / 可调试性 / 性能取舍
