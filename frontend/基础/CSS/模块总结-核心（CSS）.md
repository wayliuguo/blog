...
> 精简核心版：按篇分组、抽出高频/重要的知识点——成员丰富的主题展开明细，要点型知识点列清单、点到为止。选点按「面试命中度 / 日常复用度」等维度（见[文档组织规范](../../../文档组织规范.md)）。详细知识树见[《模块总结 · CSS》](./总结.md)。

## 知识主线（一句话）

CSS 的主轴是**布局**：01 的层叠/盒模型/BFC 决定"怎么画"，02 的 Flex/Grid 决定"怎么排"，03 的 vw/rem 决定"多端怎么变"，04 的 transform 让动效走合成层不卡，06 的现代 CSS 把覆盖顺序、设计令牌、组件级响应式升级为语言特性。
...
- **隐藏三态**：`display:none` 移除文档流触发重排、`visibility:hidden` 占位只重绘、`opacity:0` 占位且可点击走合成层
