...
- **层叠层（`@layer`）**
  - **层先于特异性**：排序是"来源与重要性 → 层 → 特异性 → 源码顺序"，同层之内才比后两者
  - **未分层 = 隐式的最后一层**：普通声明里未分层最强，这是渐进迁移能成立的原因
  - **`!important` 反转层序**：最早声明的层最强，未分层的 `!important` 最弱
  - **落地方式**：`@layer reset, vendor, components, utilities;` 写死在第一行，引入顺序不再敏感
- **自定义属性**
  - **与 Sass 变量的本质差别**：自定义属性是运行时的真实属性，可继承、可被 JS 与媒体/容器查询改写；Sass 变量编译期就被替换掉
  - **令牌能力**
    - **兜底**：`var(--x, fallback)`，名字写错也不会让整条声明作废
    - **作用域**：`:root` 是全局默认，子树里重定义就是组件级主题
    - **`@property`**：注册后可指定类型与是否继承；未注册的一律继承
  - **颜色令牌**：`color-mix()` 按比例混、`oklch()` 按感知亮度调，浅色与半透明色由主色算出来
- **容器查询**
  - **参照物是容器宽度**：媒体查询看视口，容器查询看组件自己被放多宽
  - **使用前提**：容器声明 `container-type: inline-size`（可用 `container-name` 限定），行内尺寸因此不再由内容决定，必须有确定的宽度来源
  - **分工**：设备级决策用媒体查询，组件级适配用容器查询
- **现代选择器与函数**
  - **`:is()` 带权、`:where()` 清零**：基础样式用 `:where()` 保证可覆盖，业务选择器用 `:is()` 缩短列表
  - **`:has()`**：容器按后代状态上样式，省掉一个 JS 状态类；但选择器要收窄以控制 DOM 变化时的检查成本
...
- **兼容性基线**
  ...
  - **先确认**：`@layer`、`@container`、`:has()` 是 2022～2023 才凑齐的，受监管受众用 `@supports selector(...)` 兜底
