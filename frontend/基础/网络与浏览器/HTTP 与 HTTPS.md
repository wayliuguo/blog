...
这篇文章先讲网络分层，把 HTTP 放到整个协议栈中定位——它不是孤立存在的，真正把数据搬过去的是下层的 TCP。随后我们按 传输层（TCP/UDP）→ HTTP → HTTPS → DNS → WebSocket 的顺序推进，每一步都讲清楚"为什么这一层需要存在"，最后再讲浏览器额外加的这道门：同源策略与 CORS。

沿着 HTTP/1.1 → HTTP/2 → HTTP/3 的演进脉络，你能看到队头阻塞如何一步步被解决，也能理解 HTTPS/TLS 如何用加密、证书与握手补上明文 HTTP 的安全短板。

## 一、网络分层模型与传输层

互联网通信极其复杂，没人能一次性把整个流程描述清楚。**分层**就是化繁为简的手段：把网络按不同职责切成多层，每层只关心自己的事，并向上层/下层提供稳定的接口。真实数据在发送时**从上到下逐层打包**，接收方**从下到上逐层解包**，各层互不感知对方细节。

最常提的两套模型：

- **OSI 七层模型**（理论标准）：应用层、表示层、会话层、传输层、网络层、数据链路层、物理层——分类细，但层次过碎，实践中很少严格套用。
- **TCP/IP 四层模型**（事实标准）：应用层、传输层、网络层、链路层——把 OSI 的上层合并成应用层，下面的数据链路层与物理层合并成链路层，更贴合真实网络的实现。

HTTP、TLS、TCP、IP 分别落在哪一层：
...
| 层级   | TCP/IP 四层 | 我们的主角                  |
|--------|-------------|-----------------------------|
| 应用层 | HTTP  HTTPS | 浏览器发起的请求/响应        |
| (安全层)| TLS         | 给 HTTP 加一层加密          |
| 传输层 | TCP  UDP    | 保证数据可靠/快速地送达     |
| 网络层 | IP          | 在网络上找路、寻址          |
| 链路层 | 以太网/WiFi  | 物理网卡间传输              |

注意："网络分层"是逻辑划分，真实数据是"从上到下打包、对端从下到上解包"。
```

HTTP 只是应用层协议，真正把数据搬过去的，是下层 TCP——所以我们先讲传输层。
...
讲完了把数据搬过去的传输层，接下来回到最上层的应用层：HTTP 本身。
...
- **明文传输**：数据以明文传输，可被窃听、篡改。这正是后面 HTTPS 要解决的问题（见第八节），这里不展开。
...
前面讲的都是理论，这里用配套 demo 把四类行为落到可观察的"服务端计数"上：

> 实测（运行 `npm start` 后打开 `http://localhost:5177/cache-demo.html`）
>
> 页面连同一个 /api/resource 按 mode 区分四类行为，用服务端计数证明"是否真的发了请求"：
> - `max-age=60`：第二次点击浏览器直接用本地副本、服务端计数**不增** → 强缓存命中
> - `no-cache`：每次都带条件头协商，服务端计数增加 → 每次确认
> - `ETag/If-None-Match`：第一次 200，第二次条件相同回 **304** → 复用本地
> - `Last-Modified/If-Modified-Since`：同秒回 304

该 demo 与页面同在 5177 同源，无需再启动 5178（只有跨源实验才需要第二个服务）。想观察"内容变更后重新下发新资源"，给请求加 `bump=1`（如 `?mode=etag&bump=1`）即可让服务器换一个 ETag / Last-Modified，此时同样的条件请求会返回 `200` 而不是 304。
...
- **头部压缩（HPACK）**：用静态索引表 + 动态表 + 哈夫曼编码压缩请求头，显著减少冗余头体积（详见下文）。
- **服务器推送（Server Push）**：服务器可主动推送相关资源（该特性后来因收益有限被建议弃用）。
- 仍然要求 **HTTPS**（主流实现都在 TLS 之上）。

#### 头部压缩（HPACK）展开

HTTP/2 里同一个连接往往有成百上千个请求，而它们的头大多重复（`Host`、`User-Agent`、Cookie 等几乎不变）。逐字重发纯属浪费带宽。HPACK 靠三招把大量重复的头部体积压下去：

> 示意片段（无配套脚本）

```
HPACK 靠三招把大量重复的头部体积压下去：

① 静态表：前 61 个常用头/值预置索引，发送时只发索引
   :method: GET   → 静态表索引 2   → 1 字节（原文 10+ 字节）
   :scheme: https → 静态表索引 7   → 1 字节

② 动态表：连接内第一次出现的大头（如 User-Agent），编码后加入动态表，
   同一连接后续请求直接引用该索引，不用重发全文
   第一次请求: user-agent: Mozilla/5.0 (...) → 哈夫曼编码 + 存入动态表(索引62) ≈ 60+ 字节
   第二次请求: 引用动态表索引 62                     → 1 字节
   效果：第二次请求头部从 ~60 字节降到 ~7 字节

③ 哈夫曼编码：高频字符用短编码，进一步压缩常见头值
```

落点：头部从"重发全文几十字节"降到"引用索引 1 字节"，连接越长、请求越多，省得越多。代价是 HPACK 是**连接级上下文**，动态表维护在连接两端，这也带来 CRIME/BREACH 这类压缩攻击面（其他处已提，这里呼应即可）。
...
明文 HTTP 有三个致命问题，分别对应机密性、完整性、身份认证三项缺失：

> 示意片段（无配套脚本）

```
问题① 窃听：明文路上数据能被中间人截获并读走
  Alice ──"账号:jack 密码:abc123"─(明文)─► 中间人X 截获可见 ──► Bob

问题② 篡改：数据中途被改，Bob 和 Alice 都不知情
  Alice ──"转100元"──► 中间人X 改成──"转10000元"──► Bob

问题③ 冒充：Alice 以为在跟真 Bob 通信，其实是假 Bob
  Alice ──"我是Alice,要给Bob转账"──► 假Bob(攻击者)  ← Alice 以为连上的真 Bob
```

HTTPS 要做的，就是用加密解决①、用完整性校验解决②、用数字证书解决③。
...
HTTPS 采用**混合加密**（Hybrid）——取两者之长，不走全程加密、也不全程公开：

> 示意片段（无配套脚本）

```
对称加密：同一把"会话密钥"既加密又解密 → 快，但密钥传输是难题
非对称加密：公钥加密 / 私钥解密（一对密钥）→ 安全分发，但慢

混合加密（HTTPS 的做法）：
  先用 非对称加密 安全地协商出一个"会话密钥"
  之后的大流量正文用 对称加密(AES) 加解密
  → 兼顾"分发安全" 与 "加解密性能"
```
...
> 一句话：**证书解决的是"这份公钥到底是谁的"**——没有它，非对称加密的分发环节会被冒充（对应问题③）。
...
把每一步拆开看，**关键动作**与**目的**一一对应：

| 步骤 | 关键动作 | 目的 |
| --- | --- | --- |
| ① ClientHello | 客户端声明支持的 TLS 版本与算法套件，并附随机数 A | 双方确定能共同使用的加密参数 |
| ② ServerHello + 证书 | 服务器选定算法、给出随机数 B，并下发数字证书 | 完成参数协商，并向客户端证明"我是真的服务器" |
| ③ 验证证书 → 预主密钥 | 客户端校验证书，生成"预主密钥"并用服务器公钥加密发出 | 阻止中间人冒充；预主密钥只对真正的服务器可见 |
| ④ 计算会话密钥 | 服务器用私钥解密出预主密钥，双方用预主密钥 + 随机数 A/B 各自算出相同会话密钥 | 两端独立得到同一把对称会话密钥，无需在网络上明文传递它 |
| ⑤ Finished | 双方用会话密钥加密发送"握手完成"，互相验证对方手中的密钥一致 | 确认密钥协商成功、通信双方可信 |
| ⑥ 加密通信 | 所有 HTTP 数据用会话密钥进行对称加密传输 | 此后数据机密、完整（问题①③被解决） |
...
前面十节讲的是协议本身。这一节讲的是**浏览器额外加的一道门**：同源策略（Same-Origin Policy）决定了一个页面的脚本能读哪些响应、能操作哪个 iframe 的 DOM，而 CORS（Cross-Origin Resource Sharing）是这道门上唯一的官方"放行条"。
...
- HTTP 与 HTTPS
  - 网络分层模型与传输层
    - OSI 七层 vs TCP/IP 四层（应用 / 传输 / 网络 / 链路）
    - HTTP、TLS、TCP、IP 各落在哪一层；从上到下打包、从下到上解包
  - TCP 与 UDP
  ...
  - HTTP 缓存
    ...
    - 缓存决策流程、开发实践、四类缓存实测（`cache-demo.html` + `server.js` 计数）
  - HTTP 版本演进
    - HTTP/1.1：Keep-Alive、队头阻塞
    - HTTP/2：多路复用、二进制分帧、HPACK（静态表 / 动态表 / 哈夫曼编码）
...
| 文件 | 演示什么 | 对应小节 |
| --- | --- | --- |
| `./code/site/fetch-status.html` | 通过真实 Fetch 请求回显 HTTP 状态码，并用 Promise 状态机（pending→resolved/rejected）演示请求的异步时序 | 五、状态码分类与常见状态码 |
| `./code/api-server.js` | CORS 实验用的"另一个源"（5178）：按查询参数返回不同的 `Access-Control-*` 头，并记录收到的预检请求 | 十一、同源策略与 CORS |
| `./code/site/cors-demo.js` | 从 5177 打向 5178，依次跑 8 组对照：同源基线、`*`、`Expose-Headers`、不返回 CORS 头、`credentials` 与 `*` 互斥、预检缺 `Allow-Headers`、预检补齐（页面 `cors-demo.html`） | 十一、同源策略与 CORS |
| `./code/site/cache-demo.html` | 四类缓存策略实测：max-age 强缓存（服务端计数不增证明不发请求）/ no-cache / ETag→304 / Last-Modified→304 | 六、HTTP 缓存 |
| `./code/server.js` 的 `/api/resource` 与 `/api/hits` | 缓存实验端点：按 mode 返回不同缓存头并累计真实请求次数，供页面对比计数增量 | 六、HTTP 缓存 |

启动方式：在 `code` 目录执行 `node server.js`（即 `npm start`），打开 `http://localhost:5177/`。本站点自带缓存实验（`cache-demo.html`，同源即可）与 `fetch-status.html`。`cors-demo.html` 需要**同时**启动第二个源：另开一个终端执行 `npm run api`（即 `node api-server.js`），它监听 `5178`。注意必须通过 `http://localhost:5177/cors-demo.html` 打开，用 `file://` 直接打开时页面 Origin 是 `null`，同源判定与 CORS 行为都会变。
