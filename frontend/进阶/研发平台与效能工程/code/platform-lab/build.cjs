...

{
    ...
    console.log(
        '[1] 缓存键：内容一字未改只因 mtime 变了，hash 缓存 0 重建 vs mtime 缓存 3 个全重建——云端构建必须用内容 hash（含依赖版本与构建参数）做键'
    )
}
...

{
    ...
    console.log(
        `[2] 增量依赖图：改叶子 d.js 只重建 1 个、改 c.js 重建 2 个（vs 全量 ${all} 个）；靠反向依赖边传播，且要用集合防重复入队`
    )
}
...

{
    ...
    console.log(
        '[3] 产物溯源：线上 app.8f3c.js:150 → src/cart.js:31；越界与查不到 bundle 都返回 null；真实工程用 sourcemap（VLQ 编码）做列级映射，manifest 负责「哪个版本有哪些文件」'
    )
}
