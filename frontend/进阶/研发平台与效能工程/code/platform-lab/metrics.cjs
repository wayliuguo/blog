...
function dora(deploys, changes, incidents, windowDays) {
    ...
    const mttr = incidents.length ? incidents.reduce((a, i) => a + i.recoverHours, 0) / incidents.length : 0
    return {
        deployPerDay,
        leadHours,
        failRate,
...

{
    ...
    assert.equal(
        r.grades.failRate,
        'high',
        `失败率 ${(r.failRate * 100).toFixed(1)}%：high（elite 要求 ≤ 5%，还差一档）`
    )
    console.log(
        `[1] DORA：部署 ${r.deployPerDay}/天（high）· 前置 ${r.leadHours}h（elite）· 失败率 ${(
            r.failRate * 100
        ).toFixed(1)}%（high，离 elite 的 ≤5% 差一档）· MTTR ${r.mttr}h（elite）——下一步该盯的是变更失败率`
    )
}
...

{
    ...
    console.log(
        `[2] 交付周期：72h 里编码 4h（${(coding * 100).toFixed(0)}%）、等待 ${wait}h（${((wait / total) * 100).toFixed(
            0
        )}%）——优化编码速度毫无意义，瓶颈在评审排队与发布窗口`
    )
}
...
// 把度量做成考核，人们就会优化指标本身而不是它想衡量的东西。
{
    ...
    console.log(
        `[3] 反模式：LOC 从 ${before.loc} 降到 ${after.loc}（-60%），前置时间 ${before.leadHours}h→${after.leadHours}h、故障 ${before.incidents}→${after.incidents} —— 用 LOC 考核会把这次优秀重构判成"产出下降"`
    )
    ...
    console.log(
        `[4] 反模式：同样的 200 行改动，拆成 10 个 PR 后"部署次数"从 ${honest.deploys} 变 ${gaming.deploys} —— 古德哈特定律：指标一旦成为目标，就不再是好指标`
    )
}
...

{
    ...
    for (const k of Object.keys(s)) {
        console.log(
            `      ${k}：均值 ${s[k].mean.toFixed(1)}h · 标准差 ${s[k].sd.toFixed(1)} · 变异系数 ${(
                s[k].cv * 100
            ).toFixed(0)}% · P75 ${s[k].p75}h`
        )
    }
    console.log(
        `[5] 季度曲线：Q3→Q4 降 ${q34.delta.toFixed(1)}h 但波动就有 ${q34.noise.toFixed(
            1
        )}h（信噪比 ${q34.ratio.toFixed(2)}）→ 噪声，别写进汇报；Q4→Q5 降 ${q45.delta.toFixed(
            1
        )}h（信噪比 ${q45.ratio.toFixed(2)}）→ 真变化`
    )
}
...
// 前四种是"改指标定义"，这一种是"改统计范围"，更隐蔽也更常见。
{
    ...
    console.log(
        `[6] 窗口裁剪：同样是这批变更，30 天窗口失败率 ${(rAll * 100).toFixed(1)}%，掐掉最后一周变成 ${(
            rTrim * 100
        ).toFixed(
            1
        )}% —— 跨过 elite 门槛只差一个「选哪天开始算」；固定窗口、固定口径，否则每个季度都能算出自己想要的结论`
    )
}
