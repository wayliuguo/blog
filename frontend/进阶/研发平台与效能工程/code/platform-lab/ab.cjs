...

{
  ...
  console.log(`[1] 分桶：同一用户 100 次稳定同桶；不同实验不同盐互不干扰；${N} 用户落在实验组 ${(ratio * 100).toFixed(2)}%（偏差 < 2%）`);
}
...
// 同一层内互斥（一个人只进一个实验），层与层之间用不同盐 → 正交，命中组合均匀分布。
{
  ...
  console.log(`[2] 正交分层：UI 层与排序层各自分桶，命中组合均匀分布在四格（各约 25%，最大偏差 ${(Math.max(...quad.map(c => Math.abs(c / N - 0.25))) * 100).toFixed(2)}%）——同层互斥、跨层正交`);
}
...

{
  ...
  console.log(`[3] SRM 检测：4980/5020 卡方 ${chiOk.toFixed(2)}（< 3.84 通过）；5200/4800 卡方 ${chiBad.toFixed(2)}（> 3.84 判定失衡，结论作废）`);
}
...

{
  ...
  console.log(`[4] 显著性：5.0%→5.4%（提升 8%），n=2万 时 z=${r.z.toFixed(2)} < 1.96 不显著；n=5万 时 z=${r2.z.toFixed(2)} > 1.96 才显著——样本量决定结论，不是提升幅度`);
}
