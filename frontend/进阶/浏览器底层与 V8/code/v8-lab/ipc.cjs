...
async function main() {
    ...
    console.log(
        `[1] 崩溃隔离：一个进程抛异常退出（code ${code}），另一个进程仍能正常应答 —— 这就是「一个标签页崩了不带走浏览器」的底层机制`
    )
    alive.kill()
    ...
    console.log(
        `[2] 内存不共享：子进程把 n 改成 ${back.payload.n}，父进程这边仍是 ${payload.n} —— 跨进程传值走的是结构化克隆，拿到的是副本，所以两端不会互相踩内存`
    )
    child.kill()
    ...
    for (const c of costs) {
        console.log(
            `      ${c.n.toLocaleString('en-US')} 条：克隆 ${c.clone.toFixed(1)}ms · 同进程直接遍历 ${c.pass.toFixed(
                2
            )}ms · 相差 ${(c.clone / c.pass).toFixed(0)} 倍`
        )
    }
    assert.ok(costs[2].clone > costs[0].clone * 10, '数据量涨 100 倍，克隆耗时也涨了一个量级以上')
    console.log(
        `[3] 序列化代价：1 万条克隆 ${costs[0].clone.toFixed(1)}ms，100 万条 ${costs[2].clone.toFixed(
            0
        )}ms —— 跨进程通信是**按字节付费**的，一帧 16ms 的预算里，一次大 postMessage 就能吃掉一大块`
    )
    ...
    console.log(
        `[4] 每进程固定开销：这里每个进程一启动就占 ${(per / 1024 / 1024).toFixed(
            0
        )}MB（Chrome 的渲染进程量级不同，但同构）—— 所以浏览器要按站点复用进程、并给进程数设上限，而不是一个标签页开一个`
    )
}
