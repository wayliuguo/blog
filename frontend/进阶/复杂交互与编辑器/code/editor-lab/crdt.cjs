...

{
    ...
    console.log(
        `[2] CRDT 字符模型：同一位置并发插入，两种到达顺序都为 "${toText(d1)}"——排序靠 left-origin + clientId，不靠位置`
    )
}
...

{
    ...
    console.log(
        `[3] 删除墓碑：删 b 与「在 b 后插入 x」并发，两种顺序都为 "${order1()}"；若真删则发散为 "${n1()}" / "${n2()}"`
    )
}
...

{
    ...
    console.log(
        `[4] 顺序无关：3 个客户端并发插入、全部 ${permutations([0, 1, 2]).length} 种到达顺序，结果唯一："${final}"`
    )
}
