...

{
  ...
  console.log('[1] 依赖求值：拓扑序保证 amount 在 qty/price 之后、payable 在 discount 之后；qty 3→10 时 amount 60→200、discount 0→10、payable 60→190；payable 自依赖成环在求值前抛错');
}
...

{
  ...
  console.log('[2] 联动清值：改 province 级联清空 city/district（2 次传播）；目标已为空时不传播（0 次）；互清成环不会死循环（值不再变化即收敛），代价是用户刚选的 province 被反向抹掉——配置错误而非性能问题；赋值型互推（每次写入新值）才靠深度上限拦下');
}
...

{
  ...
  console.log('[3] 异步校验竞态：连发 a/ab/abc，返回顺序 ab→a→abc，只有 abc 的结果被采纳（前两次过期丢弃），不会用旧答案覆盖新输入');
}
...

{
  ...
  console.log(`[4] 订阅粒度：100 字段表单改 1 个，全量订阅唤醒 ${allNotified} 个订阅者 vs 按字段订阅只唤醒 ${fieldNotified} 个（相差 100×）；写入相同值不触发任何通知`);
}
