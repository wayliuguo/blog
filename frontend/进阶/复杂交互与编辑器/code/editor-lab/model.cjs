...

{
  ...
  assert.deepEqual(s3, { anchorBlock: 0, anchorOffset: 2, headBlock: 2, headOffset: 0, forward: true }, '越界裁剪到块末尾');
...
{
  assert.deepEqual(tryMerge({ type: 'insert', at: 0, text: 'he', client: 'A' }, { type: 'insert', at: 2, text: 'llo', client: 'A' }), {
    type: 'insert',
    at: 0,
    text: 'hello',
    client: 'A',
  });
  assert.equal(tryMerge({ type: 'insert', at: 0, text: 'he', client: 'A' }, { type: 'insert', at: 5, text: 'x', client: 'A' }), null, '位置不相接 → 不合并');
  assert.equal(tryMerge({ type: 'insert', at: 0, text: 'he', client: 'A' }, { type: 'insert', at: 2, text: 'x', client: 'B' }), null, '不同客户端 → 不合并');
