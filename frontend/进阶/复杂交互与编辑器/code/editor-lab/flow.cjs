...

{
    ...
    console.log(
        '[1] 转移表：idle 下 send(onSuccess) 被显式拒绝且不改状态（返回 rejected 而不是静默）；合法路径 idle→loading→success→idle 正常'
    )
}
...

{
    ...
    console.log(
        '[2] 守卫与延迟：ctx.valid=false 时 onSubmit 被 guard 拦下；进入 loading 后挂 3s 超时，onCancel 时定时器被清（pending=false）；超时只在仍处 loading 时生效 → timeout'
    )
}
...

{
    ...
    console.log(
        '[3] DAG 编排：fetchUser/fetchOrder 同层并行 → 4 轮 vs 串行 5 步；pay 失败 2 次后第 3 次成功（attempts=3）；pay 彻底失败时按逆序补偿 merge/fetchOrder/fetchUser'
    )
}
...

{
    ...
    assert.deepEqual(
        first.accepted,
        ['onSubmit', 'onSuccess', 'onReset', 'onSubmit', 'onError', 'onRetry', 'onSuccess'],
        'onRetry 在 success 下无意义，被丢弃且不进历史'
    )
    for (let i = 0; i < 50; i++) {
    ...
    console.log(
        '[4] 事件重放：8 个事件里 onRetry 被拒（不入历史），重放 50 次状态恒为 success 且接受序列完全一致 → 状态机本身无隐藏可变状态'
    )
}
