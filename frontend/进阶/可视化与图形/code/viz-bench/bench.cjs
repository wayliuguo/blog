...
function startServer() {
    ...
    return new Promise(resolve => {
        child.stdout.on('data', d => {
            if (d.toString().includes('viz-bench server on')) resolve(child)
        })
    ...
    return new Promise(resolve => {
        ...
        const tick = () => {
            if (fs.existsSync(REPORT) && fs.statSync(REPORT).mtimeMs > prevMtime) {
                try {
                    return resolve(JSON.parse(fs.readFileSync(REPORT, 'utf8')))
                } catch (e) {}
            }
...
function killChrome(chrome) {
    try {
        spawnSync('taskkill', ['/F', '/PID', String(chrome.pid)], { stdio: 'ignore' })
    } catch (e) {}
    try {
        chrome.kill('SIGKILL')
    } catch (e) {}
