...
SVG 是与 Canvas 相对的另一极：**保留模式**——你画的每个图形都是 DOM 节点，浏览器替你管重绘、命中测试和动画。这一篇讲它的心智模型、D3 数据 join 的核心思想，以及什么时候它比 Canvas 更值得选。
...
- SVG 与声明式图形
  - 保留模式：DOM 即场景图，重绘/命中/缩放/可访问性全部浏览器托管
  - path 指令串：M/L/Z 折线、C 贝塞尔平滑；stroke-dash 系列做描边动画
  - 数据 join：enter / update / exit 是增量更新的统一骨架（与虚拟列表、框架 diff 同源）
  - viewBox：逻辑坐标与屏幕像素解耦，响应式近零成本
  - 交互与动画：事件绑元素、CSS 过渡几何属性
