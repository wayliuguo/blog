...
export default async function run() {
    ...
    await withServer({}, async (server) => {
        ...
        console.log(`- 两端一致：${clean.mut.mutations} 次 DOM 突变，${clean.stats.reused} 个节点全部复用，只挂监听器、只点得动`)
        console.log(`- 一处格式化口径不一致：${price.stats.patched} 个文本被静默改写 —— 不报错、只是白干，最难发现`)
        console.log(`- 一处标签写错：丢弃 ${tag.stats.discarded} 个节点、重建 ${tag.stats.created} 个，代价比一次文本改写大一个量级`)
        console.log(`- 直接重建：丢弃 ${rerender.stats.discarded} 个、重建 ${rerender.stats.created} 个，服务端那份 HTML 只剩「先看一眼」的价值`)
