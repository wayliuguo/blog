...
- HTTP/2 的并发模型
  ...
  - 请求不再受 6 连接上限约束，域名分片、雪碧图、base64 内联这些绕路手段的价值随之下降
- 两种队头阻塞
  - 应用层：HTTP/1.1 一个连接一次只能处理一个请求 → 实测快请求被前面 500ms 的慢请求压到 617ms（平均）
  - HTTP/2 用多路复用消除应用层队头阻塞 → 同样的快请求 33ms 完成
  - TCP 层：HTTP/2 全跑在一条 TCP 上，丢包波及所有 stream；HTTP/1.1 多连接时反而只影响一条
- HTTP/2 的其它变化
  - 二进制分帧是基础：HEADERS / DATA 帧按 stream 交错
  - HPACK 头部压缩是**连接级**上下文，省字节的同时带来 CRIME/BREACH 类攻击面
  - `priority` 帧实现不一致；服务端推送已被浏览器移除，用 `103 Early Hints` 替代
- HTTP/3 与 QUIC
  - 解决 TCP 层队头阻塞：stream 级重传，一条流丢包不影响其它流
  - 握手与恢复更快（TLS 1.3 内建、0-RTT 恢复），连接迁移靠 Connection ID
  - 0-RTT 数据可被重放，只用于幂等请求
  - 收益集中在弱网与移动端；必须保留 HTTP/2 回退（Alt-Svc 告知，不强制）
- 工程落地
  - HTTP/2 需要 TLS + ALPN；h2c 只用于调试
  - 判断协议版本看 DevTools 的 Protocol 列，别靠猜
  - 请求数不再是核心指标，分包依据回到缓存命中率与首屏必需；正文压缩仍必开
