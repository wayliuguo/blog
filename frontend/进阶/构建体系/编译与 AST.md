...
Babel、SWC、TS 转译、ESLint 规则、代码压缩、按需引入、自动化重构（codemod）——这些看起来毫不相干的东西，底下是同一件事：**把源码解析成树，改树，再把树变回源码**。看不懂 AST，就只能照抄插件配置、看不懂报错、改不动编译流程；看懂之后，上面所有工具都会变成"同一件事的不同用法"。

本篇用可运行的脚本把三个阶段（parse / transform / generate）逐个拆开，最后落到两个真实场景：写一个 Babel 插件，和一次批量重构。
