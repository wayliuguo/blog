...
「让 AI 写代码」和「把 AI 做进工程」是两件事。前者是个人效率工具，后者需要一套基建：**上下文工程**（喂给模型什么）、**约束输出**（怎么保证吐回来的东西能用）、**评测回归**（怎么知道改一次 prompt 没把别的搞坏）。这一篇聚焦前端视角的后两件半——结构化输出、组件生成的正确姿势、上下文预算与设计稿转代码的边界，全部用 `ai-lab` 探针给出可验证的实现。
