...

{
    ...
    console.log(
        `[1] 分片加载：80MB 模型分 ${loader.count} 片，峰值缓冲区 ${loader.peak / 1024 / 1024}MB（整包加载需 ${
            total / 1024 / 1024
        }MB）`
    )
}
...
const bytes = [
    ...
    0x7f,
    0x01,
    0x7f, // type: (i32) -> i32（payload 6 字节）
    0x03,
    0x02,
    0x01,
    0x00, // func: 1 个函数，类型 0
    0x07,
    0x07,
    0x01,
    0x03,
    ...
    0x6d,
    0x00,
    0x00, // export "sum" -> func 0
    0x0a,
    0x25,
    0x01,
    0x23, // code: 1 个函数体，长 35 字节
    0x01,
    0x02,
    0x7f, // 2 个 local：i、acc
    0x02,
    0x40, // block
    0x03,
    0x40, // loop
    ...
    0x01, // i = i + 1
    0x0c,
    0x00, // br 0（回到 loop 开头）
    0x0b,
    0x0b, // end loop; end block
    0x20,
    0x02, // local.get acc
    0x0b // end
]

{
    ...
    console.log(
        `[2] 密集计算：N=2e7 循环，WASM ${wasmMs.toFixed(1)}ms vs JS ${jsMs.toFixed(1)}ms（WASM/JS = ${(
            wasmMs / jsMs
        ).toFixed(2)}×）`
    )
}
...

{
    ...
    console.log(
        `[3] 内存预算：1B 参数 fp16 = ${(fp16 / 1e9).toFixed(1)}GB，int4 量化降到 ${(int4 / 1e9).toFixed(
            2
        )}GB（1/4）；4k 上下文 KV cache ${(kv / 1024 / 1024).toFixed(0)}MB`
    )
}
...
function block(budgetMs) {
    const t0 = process.hrtime.bigint()
    while (Number(process.hrtime.bigint() - t0) / 1e6 < budgetMs) {
        /* 模拟同步推理占用 */
    }
...

{
    ...
    console.log(
        `[4] 主线程阻塞：单帧 20ms 的推理 ×3 = ${total.toFixed(0)}ms，期间页面无法响应任何输入（应放进 Worker）`
    )
}
