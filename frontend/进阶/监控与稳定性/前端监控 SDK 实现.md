...
- 前端监控 SDK
  ...
  - 组装方式
    - 单例：避免重复初始化导致重复上报
    - 插件化：采集器之间互不感知，只共用 `emit`
  - 传输层四个必备机制
    - 攒批：12 条事件 12 次请求 → 3 次
    - 采样：入队口按比率丢弃，2000 条 @10% 接受 198 条
    - 重试：`maxRetry` 有限次重试，耗尽即弃并计 dropped
    - 溢出：`maxQueue` 丢最旧，换内存可控
  - 投递顺序
    - `sendBeacon` 优先（卸载可送达、不阻塞、无响应）
    - `fetch + keepalive` 兜底（有响应、有 64KB 上限）
  - 生命周期
    - `visibilitychange → hidden` 收口，不用 `unload`
    - LCP / CLS 越晚越准，必须在离开时定稿
  - 自身开销
    - 一次 flush 数百字节量级
    - 异步采集、热路径不做重活、错误只留栈
  - 治理口径
    - 错误不采样、埋点可低比例采样
    - URL 去 query、文本截断、不上报表单
    - 频控按指纹而非 message
