...
错误频控的键是「指纹」，不是「错误信息」：`message + 出错位置 + 行号`。原因在 `前端监控体系` 篇用实测说明——同一个函数被循环调用时，报错信息一模一样，只按 message 去重会把不同的出错点合并，只按指纹去重才既压量又不丢信息。
...
- 本模块总结：[总结](./总结.md)
- 本模块面试题：[面试题](./面试题.md)
- 上一篇：[前端监控体系](./前端监控体系.md)
- 下一篇：[前端监控实战·单页验证](./前端监控实战·单页验证.md)
