...
export default async function run() {
  ...
  const chrome = openChrome(`http://127.0.0.1:${collector.port}/track-lab.html`, ['--window-size=1280,800'])
  ...
  try {
    ...
    console.log(table(['T+Nms', '事件名', '标识', '附加字段'], events.map((e) => [
      `+${e.ts - start}`,
      e.name,
      e.id || e.url || '—',
      e.stay != null ? `stay=${e.stay}ms` : e.keyword ? `keyword=${e.keyword}` : e.ref ? `ref=${e.ref}` : e.at != null ? `at=${e.at}ms` : '—',
    ])))

    const board = aggregateSession(events, { id: events[0].sessionId })
    console.log(section('小看板：把事件流汇成可决策的数字'))
    console.log(table(['口径', '数值', '说明'], [
      ['PV', board.pv, '页面浏览量'],
