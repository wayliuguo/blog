...
import { startCollector, stopCollector, openChrome, closeChrome, waitFor, table, title, section, runAsMain } from '../harness.mjs'
...
function fakeWindow() {
  ...
  return {
    ...
    fileError() { this.dispatch('error', { target: { tagName: 'IMG', src: '/a.png' } }) },
    // 脚本错误的事件对象：target 就是 window（与资源错误区分开的关键）
    scriptError(message, lineno) {
      this.dispatch('error', { message, filename: 'app.js', lineno, colno: 5, target: this, error: new Error(message) })
    },
    rejection(reason) { this.dispatch('unhandledrejection', { reason }) },
  }
}
...
export default async function run() {
  ...
  try {
    ...
    console.log(table(['捕获方式', 'kind', '关键字段', '指纹'], errs.map((e) => [
      e.kind === 'resource' ? 'error(capture)' : e.kind === 'promise' ? 'unhandledrejection' : 'error',
      e.kind,
      e.kind === 'resource' ? fatalField(e, ['tag', 'src'])
        : e.kind === 'http' ? fatalField(e, ['url', 'status'])
          : e.kind === 'network' ? fatalField(e, ['url', 'message'])
    ...
    console.log(table(['事件', '关键字段'], [
      ['perf', perfEvent ? `fcp=${perfEvent.fcp}ms lcp=${perfEvent.lcp}ms cls=${perfEvent.cls} ttfb=${perfEvent.ttfb}ms` : '—'],
  ...
  console.log(table(['轨迹', 'kind', '指纹', '是否出端'], cap.captured.map((e, i) => [
    `#${i + 1}`,
    e.kind,
    e.fp,
    e.throttled ? '被频控拦下' : '已上报',
  ])))
