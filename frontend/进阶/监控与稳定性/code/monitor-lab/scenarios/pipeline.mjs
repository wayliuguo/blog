...
export default async function run() {
  ...
  try {
    ...
    console.log(table(['样本', 'TPS', 'avg', 'P50', 'P75', 'P95', 'P99', 'max', '错误率'], [
      [w.samples, w.tps, ms(w.avg), ms(w.p50), ms(w.p75), ms(w.p95), ms(w.p99), ms(w.max), pct(w.errorRate, 2)],
    ]))
    ...
    console.log(table(['发生时刻', '级别', '状态', '规则', '指标值', '阈值', '说明'],
      report.alerts.map((a) => [
    ...
    console.log(table(['口径', 'firing 条数', 'resolved 条数', '说明'], [
      ['有静默期（实际）', fired.length, resolved.length, '同一规则在 5000ms 内只推一次'],
      ['无静默期（假设）', naive, resolved.length, `事故秒 ${batchCount[2]} 批次 × ${fired.length} 条规则，会各推一次`],
    ]))
  } finally {
