...
export function installErrorCapture(options = {}) {
  ...
  function onError(ev) {
    ...
    if (typeof ev.message === 'string' && ev.message !== 'Script error.') {
      return push({ kind: 'script', message: ev.message, filename: ev.filename, lineno: ev.lineno, colno: ev.colno, stack: ev.error && ev.error.stack })
    }
    // 跨域脚本未带 crossorigin，浏览器只给一句 "Script error."，细节全被抹掉
    return push({ kind: 'cross-origin', message: ev.message || 'Script error.', filename: ev.filename, lineno: ev.lineno })
  }
