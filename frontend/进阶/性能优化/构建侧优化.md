...
- 构建侧优化
  - 三个方向
    - 更少：代码分割、路由懒加载、按需引入
    - 更小：tree-shaking、压缩、依赖替换、现代语法
    - 更合适：传输压缩、sourcemap 策略、chunk 划分与哈希
  - 代码分割
    - 实测（1.5 Mbps 限速）：首屏全量 2069ms vs 按需 767ms，总字节几乎相同（347.6KB vs 348.2KB）
    - 收益是「把下载时刻从首屏挪到需要时」，不减少总量
    - 首屏路由与高频路由不该懒加载；低频页面适合，且必须配 loading 兜底
  - tree-shaking
    - 三个前提：ESM 静态结构、无副作用（`sideEffects: false`）、静态访问
    - CJS 的 `require` 无法静态分析，整块保留
    - 只摇「没被引用的导出」，不摇「没执行到的分支」
  - 按需引入
    - 实测 10 个特性只用 3 个：整包 52.6KB vs 按需 19.1KB（砍 63%）
    - 三个层级：入口级（路由）、模块级（动态 import + sideEffects）、依赖级（组件库子路径、换小替代、polyfill 按需）
  - 压缩与产物
    - gzip 必开，brotli 再小 15%~20%，只对文本有效
    - 生产环境不部署 sourcemap，改为上传监控平台供错误反解
    - vendor 拆分 + 内容哈希提升缓存命中率，但别拆太碎
  - 确认改对了
    - 逐 chunk 的 gzip 体积 diff + Coverage 复核，能抓出「插件没生效」
    - 最后回到首屏资源清单与真实指标
