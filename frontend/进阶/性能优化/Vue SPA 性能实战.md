...
- Vue SPA 性能实战
  - 实验台形态
    - 一个真的 Vue 3 SPA（hash 路由 + 3 个路由模块 + 2000 条订单），两版外壳对应两套决策
    - 每个手段是独立开关（`?opt=1` 全开，也可单项覆盖），否则无法归因
    - 本机 localhost 是"秒下"，测加载类对比必须人为加延迟，否则读数完全一样
  - 第一段 · 加载
    - 关键 CSS 内联 + 全量 CSS 异步（`media="print"`）：省一跳阻塞；内联部分必须含布局尺寸，否则用 CLS 换 FCP
    - `modulepreload` 省掉"解析到入口才知道要下 vue"的串行一跳
    - 骨架屏：FCP 508 → 224 ms、首屏有内容 247 → 94 ms、关键路径 JS 10 → 7 个
    - 陷阱：LCP 落在骨架屏上（336 ms），指标变绿但用户还没看到内容 → 必须配自定义"内容就绪"指标
    - 路由级分割省的是业务代码不是体积（152.2 → 149.1 KB，Vue 运行时占 131.5 KB）
  - 第二段 · 切路由
    - 全量打包 69 ms / 分割不预取 245 ms / 分割 + 预取 80 ms（150 ms RTT 下）
    - 只做分割是亏的：拿 176 ms 切换等待换 3 KB 首屏；预取必须成对做
    - 预取占的是空闲带宽与用户流量，按访问概率决定，低频路由才拆
  - 第三段 · 运行时
    - 排序交互到下一帧：134 ms（2000 行全量）→ 31 ms（虚拟滚动）→ 28 ms（+ 分片）
    - 虚拟滚动省"渲染"，分片省"算"；算没超过 50 ms 时分片是纯亏（总耗时会变长）
    - 滚动回调：每个 scroll 逐行读写 3985 ms / 最长任务 3035 ms → rAF 里读写分离 1007 ms / 0 个
    - 改法是读写分离，不是"少滚几次"
    - 口径：无头环境没有真实输入，`click()` 不产生 event 条目，INP 只能近似，线上必须靠 RUM
  - 第四段 · 缓存
    - 无缓存 289 → 278 ms；HTTP 强缓存 266 → 122 ms（传输 0 B）；SW 276 → 205 ms（传输 0 B）
    - SW 比 HTTP 缓存慢 ~80 ms：冷启动线程 + 每个请求过一遍 fetch 回调
    - SW 的价值在离线与按资源分策略，不是"更快的缓存"；顺序是先 HTTP 缓存再考虑 SW
    - 前提都是文件名带内容 hash
    - 实验坑：HTTP 缓存按 origin 存，端口随机会导致"缓存没生效"的假象，缓存场景必须固定端口
  - 第五段 · 闭环
    - 预算分级：确定性指标（体积 / chunk 数 / 请求数）卡死，统计性指标（耗时）多轮中位数 + 20% 余量
    - "关键路径 JS 数"最容易被业务需求破坏（改回静态引入），要单独卡
