...
前面两篇都在讲「尽快把东西送到用户眼前」。但用户真正不爽的时刻往往发生在**页面已经打开之后**：点了没反应、滚动掉帧、切几次页面越用越卡。

运行时性能有两个标尺，把这两个数记住，后面所有手段都能归位：

- **单帧 16ms**：要维持 60fps，每一帧的所有工作（脚本 + 样式 + 布局 + 绘制）必须挤进 16.7ms。任何一段超了，就是掉帧。
- **长任务 50ms**：超过 50ms 的任务会被浏览器标记为长任务（long task）。长任务期间用户输入无法被处理——这是 INP 变差的直接原因。

这一篇按「谁占住了主线程」来组织，每个手段都配实测。

## 一、强制同步布局：读写交错为什么能慢一千倍
...
所以动画要尽量只用 `transform` 和 `opacity`——它们能在合成层上完成，不触发布局与重绘。这同时解释了上一篇 CLS 里的那条规则：`transform` 不改变元素在布局中的起始位置，所以不产生位移。
...
排查路径见「性能测量工具实操」篇（Heap snapshot Comparison、Detached DOM、Allocation timeline）。
...
- 运行时性能
  ...
  - 长任务与切片
    ...
    - 切片只能切独立工作单元，单个超长任务切不动
  - 重排重绘与合成
    - 几何属性 → 布局；绘制属性 → 重绘；`transform` / `opacity` → 只合成
    - 动画只用 `transform` / `opacity`；`will-change` 别滥用
  - 长列表
  ...
  - 内存
    - 实测留引用净涨 18.3MB vs 可回收 0.8MB
    - 四类常见泄漏：事件监听、定时器、只增不减的全局缓存、闭包绑大对象
...
| 文件 | 作用 | 对应小节 |
| ---- | ---- | ---- |
| `./code/perf-lab/pages/thrash.html` | 2000 个盒子：交错读写 vs 集中读写 | 一 |
| `./code/perf-lab/pages/longtask.html` | 60000 行列表：一次做完 vs 分片切片 | 二 |
| `./code/perf-lab/pages/virtual.html` | 10000 行长列表：全量渲染 vs 虚拟滚动 | 四 |
| `./code/perf-lab/pages/worker.html` | 同一段计算放主线程 vs 放 Worker | 五 |
| `./code/perf-lab/pages/heavy.js` | 主线程与 Worker 共用的热点计算函数 | 五 |
| `./code/perf-lab/pages/heavy-worker.js` | Worker 线程入口，算完把结果回传 | 五 |
| `./code/perf-lab/pages/memory.html` | 分配 / 留引用 / 采样堆大小的最小对照 | 六 |
| `./code/perf-lab/scenarios/thrash.mjs` | 循环耗时与强制布局次数对照 | 一 |
| `./code/perf-lab/scenarios/longtask.mjs` | 最长同步块与首屏可见耗时对照 | 二 |
| `./code/perf-lab/scenarios/virtual.mjs` | DOM 节点数、首屏渲染、滚动与堆对照 | 四 |
| `./code/perf-lab/scenarios/worker.mjs` | 最长帧间隔与期间出帧数对照 | 五 |
| `./code/perf-lab/scenarios/memory.mjs` | 堆增长对照（需要 `--enable-precise-memory-info`） | 六 |
