...
export default async function run() {
    ...
    if (main && worker) {
        console.log(
            `- 主线程版：计算期间浏览器 ${ms(main.report.extra.maxGap)} 没能刷出一帧，页面直接冻住 ${ms(
                main.report.extra.maxGap
            )}`
        )
        console.log(
            `- Worker 版：计算挪到另一个线程，主线程最长只被挡住 ${ms(worker.report.extra.maxGap)}，期间照常出帧 ${
                worker.report.extra.beats
            } 次`
        )
    ...
    console.log(
        '- 通信要序列化：postMessage 传的是结构克隆，传大对象本身有成本，适合传「输入参数 / 结果」，不适合高频传大块数据'
    )
