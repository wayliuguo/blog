...
export default async function run() {
    const rows = await sweep('/memory.html', [
    ...
    console.log(
        '- 可回收版：数组出了函数作用域就没人引用，分配压力上来时 V8 触发 GC 把它们收走 → 堆在一条水平线附近波动'
    )
    ...
    console.log(
        '- DevTools Memory 面板：先打一次堆快照 → 反复操作要排查的动作 → 再打一次 → 选 Comparison 看哪些对象只增不减'
    )
    console.log('- Detached DOM：筛 "Detached" 关键字，能看到「已经不在页面里、但被 JS 引用着」的节点')
    console.log(
        '- Allocation instrumentation on timeline：录一段操作，蓝色条是新增分配，灰色条是已被回收，全是蓝的说明泄漏'
    )
}
