...
export default async function run() {
    ...
    const totalOf = report =>
        report.resources
            .filter(r => r.name.includes('/bundle') || r.name.includes('/feature'))
    ...
    for (const { name, report } of rows) {
        console.log(
            `[${name}] 首屏 ${ms(report.extra.loadMs)}，整个会话共下发 ${report.extra.chunks} 个 chunk / ${bytes(
                totalOf(report)
            )}`
        )
    }
    ...
    console.log(
        '- 按需版首屏只下当前路由，首屏更快；代价是切到没去过的路由时，会出现一次短暂的加载等待（需要 loading 兜底）'
    )
