...
export default async function run() {
    ...
    console.log(
        table(
            ...
            rows.map(({ name, report }) => [
                ...
                report.extra.blocks ? report.extra.blocks.filter((b) => b > 50).length : report.extra.longestBlock > 50 ? 1 : 0
            ])
        )
    )
    ...
    for (const { name, report } of rows) {
        console.log(`[${name}] 总耗时 ${ms(report.extra.totalMs)} · 最长同步块 ${ms(report.extra.longestBlock)} · 首屏可见 ${ms(report.extra.firstContentMs)}`)
    }

    const [sync, chunked] = rows
    if (sync && chunked) {
        console.log(
            `\n结论：切片版总耗时更长（多 ${ms(chunked.report.extra.totalMs - sync.report.extra.totalMs)}，因为每次结算布局都有固定开销），`
        )
        console.log(
            `但最长同步块从 ${ms(sync.report.extra.longestBlock)} 降到 ${ms(chunked.report.extra.longestBlock)}，`
        )
        console.log(
            `首屏可见耗时从 ${ms(sync.report.extra.firstContentMs)} 降到 ${ms(chunked.report.extra.firstContentMs)}——用户不用等全部渲染完。`
        )
    }
    ...
    console.log('- 总耗时变长一点没关系，只要没有长块，交互就能一直跟手；反过来，总耗时再短，一个 300ms 的块也会让点击「没反应」')
