...
printTable('场景一：只改 cart.items（1 个组件该动，另外 2 个不该动）', [
    {
        ...runStoreRoute(
            ...
            () => state => state,
            store => store.dispatch({ type: 'cart/setItems', items: 1 })
        ),
        name: 'reducer（订阅整棵树）',
    ...
    },
    {
        ...runStoreRoute(
            create(() => ({ ...initialState })),
            key => selectors[key],
            store => store.setState({ cart: { items: 1 } })
        ),
        name: 'setState + 选择器',
...
printTable('场景二：无关更新 / 赋同值（三个组件都不该动）', [
    {
        ...runStoreRoute(
            createStore(rootReducer, initialState),
            key => selectors[key],
            store => store.dispatch({ type: 'nobody/cares' })
        ),
    ...
    },
    {
        ...runStoreRoute(
            create(() => ({ ...initialState })),
            key => selectors[key],
            store => store.setState({ theme: 'light' })
        ),
        name: 'setState 赋同值',
...
printTable('场景四：一轮里连续 setState 10 次', [
    ...
    },
    {
        ...runStoreRoute(
            create(() => ({ ...initialState })),
            key => selectors[key],
            store => store.setState(state => ({ cart: { items: state.cart.items + 10 } }))
        ),
