...
E2E 是唯一一层在**真实浏览器**里跑的测试：真实的网络、真实的存储、真实的渲染与事件。它能抓到前面几层永远抓不到的问题 —— Safari 下的日期格式、iOS 输入法的 composition 事件、跨页面的状态丢失。代价是慢、贵、脆。本篇的目的很明确：让 E2E **写得出、跑得稳、管得住**。围绕这个目的分两层展开：先讲「怎么写稳」——守什么路径、用什么定位器、怎么自动等待、怎么控数据与环境；再讲「为什么 flaky 以及怎么治理」——四个可复现根因、规模化手段、治理流程。Playwright 段为结构示例，flaky 实验可实跑。
