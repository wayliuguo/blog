...
E2E 是唯一一层在**真实浏览器**里跑的测试：真实的网络、真实的存储、真实的渲染与事件。它能抓到前面几层永远抓不到的问题 —— Safari 下的日期格式、iOS 输入法的 composition 事件、跨页面的状态丢失。代价是慢、贵、脆。本篇解决两件事：E2E 怎么写才不脆，以及 flaky（时红时绿）到底从哪来、怎么根治。
...
- E2E 与稳定性
  - E2E 守什么
    ...
    - 不放：组件分支、边界值、表单校验组合、视觉细节
    - 规模控制在几十条，超过就变成负担
  - 写法要点
    - 定位器语义化：`getByRole` / `getByLabel` / `getByText` 优先
    - 每用例自带完整前置，不共享登录态
    - URL 用正则匹配，不用字符串相等
  - 自动等待
    - web-first 断言会自动重试到超时
    - **不写 `waitForTimeout`**：慢机器不够、快机器白等
    - 等条件：URL / Response / 元素可见
  - 数据准备
    - 每用例独立数据，避免并发互踩
    - 前置数据用 API 造，不靠 UI 走一遍
    - 第三方依赖用 `page.route` 拦截；自己系统的接口不拦
  - flaky 四个根因
    - **sleep 赌时间** → 等条件成立
    - **用例间共享状态** → beforeEach 重置 / 独立构造，用 shuffle 暴露
    - **依赖"现在"** → 时间注入 + `setSystemTime`
    - **时区** → 明确业务口径，固定 `TZ` 或用 UTC 方法
    - 反例要写成确定性的（`it.fails`），否则反例自己也是 flaky
  - 规模化
    - 并行 worker 的前提是数据独立
    - 分片按稳定顺序切，失败才保留 trace
    - `retries` 只降噪，不治本
  - 治理流程
    - 量化失败率 Top10 → 隔离 → trace 定位 → 修根因 → 重复跑验证 → 立规矩
...
| 文件 | 作用 | 对应小节 |
| --- | --- | --- |
| `./code/test-lab/src/flaky/streak.js` | 被测源码：签到连续天数 + 共享访问记录 | 五、flaky 的四个根因 |
| `./code/test-lab/src/flaky/flaky.test.js` | 四个根因的复现与修法（含 `it.fails` 反例） | 五、flaky 的四个根因 |
| `./code/test-lab/package.json` | `test:flaky`（默认顺序）/ `test:shuffle`（打乱顺序） | 五、flaky 的四个根因 |
