...
{
    assert.equal(
        grade({ coreBroken: true, affectedUsers: 50, moneyLoss: 0, workaround: true }),
        'P0',
        '核心不可用，哪怕只有 50 人'
    )
    assert.equal(
        grade({ coreBroken: false, affectedUsers: 3, moneyLoss: 12000, workaround: true }),
        'P0',
        '有资损即 P0'
    )
    assert.equal(grade({ coreBroken: false, affectedUsers: 50000, moneyLoss: 0, workaround: true }), 'P1', '大范围')
    assert.equal(
        grade({ coreBroken: false, affectedUsers: 300, moneyLoss: 0, workaround: true }),
        'P3',
        '有绕过方案 → 降级'
    )
    assert.equal(grade({ coreBroken: false, affectedUsers: 300, moneyLoss: 0, workaround: false }), 'P2', '无绕过方案')
    console.log(
        '[1] 事故分级：核心不可用或有资损 → P0（与影响人数无关）；大范围或核心降级 → P1；局部且能绕过 → P3；定级决定响应节奏，不是先讨论再定级'
    )
}
...

{
    ...
    console.log(
        `[2] 时间线：MTTD 35min + MTTR 15min = 50min（瓶颈在发现）；补监控后 MTTD 5min、MTTR 仍 15min —— 先分清慢在哪一段，再决定改监控还是改预案`
    )
}
...

{
    ...
    assert.ok(
        r.open.some(x => x.includes('逾期')),
        '第 12 天到期、现在是第 20 天 → 逾期 8 天'
    )
    assert.ok(r.open.some(x => x.includes('缺 owner')))
    assert.ok(
        r.open.some(x => x.includes('缺截止时间')),
        'dueDay 为 0 视为未填'
    )
    assert.ok(r.open.some(x => x.includes('缺验收标准')))
    assert.equal(r.open.length, 4, '五条行动项里四条有问题')
    console.log(
        '[3] 复盘行动项：owner / 截止时间 / 验收标准 三缺一即判不合规；实测 5 条只闭环 1 条（20%），1 条逾期 8 天、另三条分别缺 owner / 缺截止时间 / 缺验收标准'
    )
}
