...

{
  ...
  console.log(`[1] CR 检查器：改公共接口 / 加依赖 / 动构建配置 / 大改动 四条规则自动判，实测命中 ${r1.blockers.length} 条 blocker；能自动判的别浪费评审人力`);
}
...

{
  ...
  console.log(`[2] 技术债打分：影响面 × 恶化速度 ÷ 修复成本 —— 「散落的工具函数」6.0 分排第一（便宜且在扩散），「遗留 jQuery」0.8 分垫底（贵但不恶化）`);
}
...

{
  ...
  console.log('[3] 重构安全网：测试 / CI / 回滚 / 监控 四件套；缺 0 项可做大规模重构、缺 1 项只做小步改动、缺 ≥2 项先补网——没有网的重构就是赌博');
}
