...
function renderNode(node, scope, stats) {
    ...
    return {
        ...
        children: (node.children || [])
            .map(c => renderNode(c, scope, stats))
            .filter(Boolean)
            .flat()
    }
}
...

{
    ...
    assert.deepEqual(
        tree.children[1].children.map(c => c.props.text),
        ['a', 'b']
    )
    ...
    console.log(
        `[2] 递归渲染：条件开关生效（顶层 2 → 1 个），循环展开 ${tree.children[1].children.length} 份且 item 变量可见`
    )
}
...

{
    ...
    console.log(
        `[3] 更新粒度：全量渲染访问 ${all.visited} 个节点，只重算脏子树 ${patch.visited} 个（省 ${
            all.visited - patch.visited
        } 个）`
    )
}
