...
- 新增脚本时必须同步三处：项目 README、`package.json` 的 `scripts`、对应篇正文的配套代码表（见《文档组织规范》的「代码片段与配套代码表」一节）。

## 内容规范

文档组织与写作规则见仓库根 [`文档组织规范`](./文档组织规范/readme.md)，要点：

- 板块 / 模块 / 配套代码目录结构，文件名编号仅作文档 ID，真实阅读顺序以侧栏为准；
- 单篇结尾顺序固定：正文 → 配套代码 → 参考；
- 正文代码块逐字来自标注的配套脚本，无脚本的速查片段显式标注「示意片段」；
- 模块总结先列大纲、再逐项展开（全文按知识体系章节分层收纳）；
...
文档层面的链接、侧栏覆盖、代码同步等检查与判定标准见《文档组织规范》的「改动后的校验」一节。
