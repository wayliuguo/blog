...
> 对应 [WebSocket 实时通信](../NestJS%20进阶/WebSocket%20实时通信)：没有这段配置，前端 WebSocket 会握手失败。
...
| 相关文档 | 衔接点 |
|---------|--------|
| [PM2 进程管理](./PM2%20进程管理) | Nginx 背后就是 PM2 管的 Node 进程 |
| [Docker Compose 编排](./Docker%20Compose%20编排) | compose 里端口绑定 `127.0.0.1`，正是为了只让 Nginx 访问 |
| [生产部署实战](./生产部署实战) | 完整架构如何落地 |
...
- 本模块总结：[总结](./总结.md)
- 本模块面试题：[面试题](./面试题.md)
- 上一篇：[Docker Compose 编排](./Docker%20Compose%20编排)
- 下一篇：[数据库迁移与发布](./数据库迁移与发布)