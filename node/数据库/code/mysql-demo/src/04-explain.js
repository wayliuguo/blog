...
const cases = [
    ['等值查询：按 email 查找（唯一索引应命中）', "SELECT * FROM users WHERE email = 'user50000@example.com'"],
    ['范围查询：主键 id 区间（应走 PRIMARY，type=range）', 'SELECT * FROM users WHERE id BETWEEN 1000 AND 2000'],
    [
    ...
    ['覆盖索引：只查联合索引包含的列，无需回表', 'SELECT user_id, created_at FROM orders WHERE user_id = 50000'],
    ['SELECT * 破坏覆盖：同样条件但查全列，需回表', 'SELECT * FROM orders WHERE user_id = 50000'],
    [
        '函数作用在索引列：UPPER(email) 使 uq_email 失效',
        "SELECT * FROM users WHERE UPPER(email) = 'USER50000@EXAMPLE.COM'"
    ],
    ["左模糊 like '%x'：前缀模糊使索引失效", "SELECT * FROM users WHERE email LIKE '%example.com'"],
    ['低区分度字段：status 无单列索引，全表扫描', 'SELECT * FROM users WHERE status = 1']
]
