...
| 知识点 | 一句话结论 | 出处 |
| --- | --- | --- |
| 最左前缀原则 | 联合索引 `(a, b)` 只支持 `a` 与 `a + b` 的查询，单查 `b` 用不上索引 | 数据库/02 篇 |
| 索引失效的根因 | 索引列在 WHERE 里被"改造"——函数、运算、类型转换、`%x` 前缀模糊都会让 B-Tree 失去有序性 | 数据库/02 篇 |
| 快照读与当前读 | 普通 SELECT 读 MVCC 快照且不加锁；UPDATE / DELETE / `FOR UPDATE` 是当前读并加锁 | 数据库/02 篇 |
| 防超卖的原子 UPDATE | 把 `balance >= 1` 下推进 UPDATE 的 WHERE，再用 affectedRows 判定成败 | 数据库/02 篇 |
| Binlog 三种格式 | 生产用 ROW；STATEMENT 遇到非确定性函数会导致主从不一致 | 数据库/03 篇 |
| 分片键选型 | 按时间分片必然写入热点，user_id / order_id 哈希取模才分布均匀 | 数据库/03 篇 |
| 连接池连接数估算 | 约等于 `CPU 核数 × 2 + 磁盘数`，4 核 8G 建议 20-50，绝非越多越好 | 数据库/03 篇 |
| 参数化查询 | mysql2 用 `?` 占位符传参，绝不用模板字符串拼 SQL | 数据库/04 篇 |
| N+1 查询 | 循环里查关联表会变成 1+N 次；用 `relations` 或 `leftJoinAndSelect` 压成一次 | 数据库/04 篇 |
| 嵌入与引用 | 一起查、量小、不单独变化用嵌入；独立变化、量大用引用加 `populate` | 数据库/05 篇 |
| TTL 索引 | `expireAfterSeconds` 让文档自动过期，后台每 60 秒扫描一次 | 数据库/05 篇 |
| 聚合管道 | `$match → $group → $sort` 是主干，`$lookup` 等价 LEFT JOIN，`$unwind` 展开数组 | 数据库/06 篇 |
| 副本集选举 | 心跳超时默认 10 秒触发，需过半数票，所以节点数取奇数 | 数据库/06 篇 |
| JSON 与 JSONB | JSONB 二进制存储、可直接取字段且能建 GIN 索引，JSON 三样都做不到 | 数据库/07 篇 |
| UPSERT 幂等写入 | `ON CONFLICT (...) DO UPDATE SET ... = EXCLUDED....`，一条语句避开先查后写的竞态 | 数据库/07 篇 |
| VACUUM 与表膨胀 | PG 的旧行是表内 dead tuple、必须 VACUUM 回收；MySQL 是 undo log 加 purge 线程 | 数据库/07 篇 |
| 相似度检索 | 存 `VECTOR(1536)`，用余弦距离 `<=>` 取 TopK；小数据 IVFFlat，新项目 HNSW | 数据库/07 篇 |
| RDB 与 AOF 的取舍 | RDB 恢复快但可能丢数据，AOF 最多丢 1 秒但文件大；混合持久化两者兼得 | Redis/02 篇 |
| 内存淘汰策略 | 纯缓存用 `allkeys-lru`，只想淘汰带 TTL 的 key 用 `volatile-*`，默认 `noeviction` 直接报错 | Redis/02 篇 |
| Big Key | String 超过 10KB 或集合超过 5000 元素即为大 key；拆分后用 `UNLINK` 异步删 | Redis/02 篇 |
| 缓存三大问题 | 穿透缓存空值或加布隆过滤器；击穿用 `SET NX EX` 互斥锁；雪崩靠 TTL 随机化加预热 | Redis/04 篇 |
| Cache Aside | 写路径是"先写库再删缓存"，删而不是更新，把不一致窗口压到最小 | Redis/04 篇 |
| 分布式锁 | `SET NX EX` 加锁，解锁要用 Lua 比对 value；多节点场景用 Redlock | Redis/04 篇 |
| Cluster 哈希槽 | 共 16384 个槽，`CRC16(key) % 16384` 定位节点，支持自动分片与水平扩展 | Redis/05 篇 |
...
上一模块是 [模块三 · Web 框架基础](../Express%20与%20Koa/总结.md)，它给了本模块三样前提：一是 HTTP 请求生命周期与中间件模型，知道一次请求在应用里经过哪些层；二是 Controller / Service 的分层约定，数据访问该放在哪一层已有共识；三是"外部依赖由容器注入"的写法，数据库连接在上一模块里只是一个待注入的对象。本模块做的事，就是把那个对象真正填实。

**本模块交给下一模块什么**

下一模块是 [模块五 · 后端架构](../NestJS%20入门/总结.md)，它要靠本模块的成果：ORM 的实体定义与迁移、缓存作为独立基础设施的接入方式、事务与幂等写入的落地代码、连接池与 Redis 客户端的参数配置与监控口径。换句话说，本模块交付的不只是"会用某个库"，而是**数据层的能力边界**——哪些操作便宜、哪些昂贵、哪些根本做不到（跨分片 JOIN、Redis 事务回滚、PG 表膨胀），这些边界决定了下一模块架构方案的上限。

## 代码与自测入口

本模块有两个配套代码项目，路径如下（文本给出，命令行里直接 cd 进去）：

- `node/数据库/code/mysql-demo`：建表 SQL 与 CRUD、索引对比、EXPLAIN 读法、事务与防超卖、乐观锁与悲观锁。按 README 的「3 步跑起来」执行即可，脚本清单里每个文件对应本模块的一个知识点。
- `node/Redis/code/redis-demo`：数据类型用例、TTL 与过期行为、Cache-Aside 读写、分布式锁与 Lua 安全释放、限流、排行榜、发布订阅。按 README 的「2 步跑起来」执行。
...
- [01-Redis 基础与数据类型](../Redis/01-Redis%20基础与数据类型.md)
- [02-Redis 持久化与淘汰策略](../Redis/02-Redis%20持久化与淘汰策略.md)
- [03-Node.js 操作 Redis](../Redis/03-Node.js%20操作%20Redis.md)
- [04-Redis 缓存实战](../Redis/04-Redis%20缓存实战.md)
- [05-Redis 进阶](../Redis/05-Redis%20进阶.md)

其他：

- [模块面试题](./面试题.md)
- 上一模块：[模块三 · Web 框架基础](../Express%20与%20Koa/总结.md)
- 下一模块：[模块五 · 后端架构](../NestJS%20入门/总结.md)
