// 06 错误的三层防线：同步 try/catch、异步 Promise.catch、进程级兜底
// 对应文档：node/运行环境/06-进程线程与优雅退出.md
// 演示：同步 / 异步 / 进程级三道防线，以及「没有兜底会怎样、有兜底会怎样」
// 运行命令：
//   node src/06-error-fallback.js            # 默认：只演示前两道防线（不会让进程崩掉）
//   node src/06-error-fallback.js uncaught   # 演示 uncaughtException 进程级兜底
//   node src/06-error-fallback.js unhandled  # 演示 unhandledRejection 进程级兜底
...
async function main() {
    ...
    console.log('    node src/06-error-fallback.js uncaught   # 看 uncaughtException')
    console.log('    node src/06-error-fallback.js unhandled  # 看 unhandledRejection')
