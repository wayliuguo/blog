...
String 适合简单键值对（如缓存 JSON 字符串）、计数器（如访问量、点赞数）。Hash 适合存储对象（如用户信息、商品详情），可以单独操作某个字段而不影响其他字段，节省空间。

### Q2: Redis 为什么是单线程的，单线程如何保证高性能？

单线程避免上下文切换和锁竞争，所有命令串行执行天然无并发问题。高性能来源于：
1. 基于内存存储，读写都是微秒级
2. IO 多路复用处理大量客户端连接
3. 高效数据结构设计（SDS、跳表、压缩列表）

### Q3: Redis 为什么是 16384 个哈希槽？

16384 是 2^14，用 14 位就能表示，足够绝大多数场景。如果槽太少，每个节点数据量太大；槽太多，维护开销大。16384 是经验值选择。

### Q4: String 为什么用 SDS 而不是 C 语言字符串？

SDS（Simple Dynamic String）相比 C 字符串：
1. 保存了长度信息，获取 O(1)
2. 减少内存重新分配次数（预分配机制）
3. 兼容部分 C 字符串函数
4. 二进制安全（可以存储任意二进制数据）

### Q5: Redis 适用哪些场景？不适用哪些场景？

**适用**：热点数据缓存、计数器、排行榜、分布式锁、限流、会话共享、消息队列。
**不适用**：需要持久化存储大量数据（受内存限制、成本高）、需要强事务、复杂多表关联查询。
