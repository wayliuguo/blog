    ...
    return new Promise(resolve => {
        ...
        fs.readFile(tmp, (err, data) => {
            ...
            fs.writeFile(out1, data, () => {
                ...
                console.log(
                    `  进程 RSS(约): ${Math.round(process.memoryUsage().rss / 1024 / 1024)}MB（含整文件 200MB 缓冲）`
                )
                resolve()
            })
        })
...
function copyByStream() {
    return new Promise(resolve => {
        ...
        ws.on('finish', () => {
            ...
            console.log(
                `  进程 RSS(约): ${Math.round(process.memoryUsage().rss / 1024 / 1024)}MB（分块读写，峰值稳定）`
            )
            resolve()
        })
    })
