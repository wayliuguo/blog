...
主表：其主键被另一个表的外键关联的表，多对多时除了中间表都是主表

从表：有外键外联了一个表的表

| 方向          | cascade 仅「主控端（从表）→ 被控端（主表）」配置生效，反向操作无联动 |
| ------------- | ------------------------------------------------------------ |
| 主表操作→从表 | 配置 cascade: true 时，删除 / 更新主表会自动删除 / 更新从表关联数据；未配置则报错（外键约束） |
| 从表操作→主表 | 无论是否配置 cascade，删除 / 更新从表都不会影响主表数据      |
| 多对多特殊    | cascade 仅影响中间表，不删除另一主表数据                     |
