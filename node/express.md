...
## express 简单实现
- `createApplication` 是一个函数，具有`listen`、`use`、`all`及其他`method`上的方法
- `use`、`method`、`all` 收集`method`、`path`、`handler`在`app.routes`数组中
- `listen` 内部执行了`http.createServer`开启了一个服务端，其回调函数为`app`方法
- `app 方法原理`
  - 实现一个`next`方法，每当匹配到对应的`method`和`path`时，就执行对应`handler`
```
