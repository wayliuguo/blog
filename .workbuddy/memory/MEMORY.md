...
## 结构
VitePress（`base:'/blog/'`，构建 ~55s），板块 `ai/ frontend/ interview/ node/`，配置 `.vitepress/config/*.js`，产物恒 `.vitepress/dist`。`node/` 11 模块、`frontend/基础/` 8 模块（含「工程协作」）、`frontend/进阶/` 19 模块（模块清单与顺序以侧边栏配置为准）：**19 个模块全部落地，无占位模块**：微前端体系 5、可视化与图形 5、浏览器底层与 V8 4、跨端与桌面深化 4、AI 前端工程 4、低代码与搭建体系 4、复杂交互与编辑器 4、研发平台与效能工程 3、工程素养 3（L3）。另散篇 `面试方法论.md` 为**板块级独立入口**；frontend 全板块 198 篇含元页面。**目录与文件名不带编号**，NN- 只用于 总结.md 分篇标题与「见第 X 篇」阅读序标签，侧边栏是唯一顺序来源。

## 必须知道的坑
- **围栏**：`text`/`ejs`/`plaintext` 都会 shiki 告警（`txt` 是别名不告警），ASCII 图、模板示例、实测输出一律用裸围栏（2026-09-19 全站清零 11 处）。
...
- **相邻两段同源代码块必须各写一次 `> 摘自`**：check-code-sync 只看围栏上方 4 行，第二个块拿不到第一个块的标注 → 报「缺出处标注」（同文件重复标注允许）。**同文件重复标注允许，跨篇引用同一文件也允许**（校验按篇独立）。
- **`> 摘自 <path>` 的脚本必须出现在该篇 `## 配套代码` 表里**，否则报「表里没有」（`tablePaths()` 只认第一列含 `/` 的行）。新增一处 `> 摘自` 就要顺手补表。**文档代码块比源码多一个尾逗号/少一行也挂**（骨架里逗号是有效字符）——实测踩到：正文写 `maxQueue: 100,`（源码为 `maxQueue: 100`）报「代码对不上」。
...
**端口**：5174 HTML ·5175 CSS ·5176 JS ·5177 网络(+api 5178)·5179 工程化 ·5180 React·5181 Vue·5182 框架原理 ·5183 交付 ·5184 工程实践 ·5185 小程序 ·5186 TS·5187 性能 ·**5188+5189 都是 monitor-lab 的 `enterprise-server.js`**·5190 网络与协议进阶 ·**5191 渲染架构(render-lab)**。纯 CLI 探针不占端口：`esm-probe`、`mini-react`/`mini-vue`/**`mini-store`**/**`type-gym`**/**`runtime-lab`**/**`sandbox-lab`**/**`mf-demo`**/**`mini-micro`**/**`viz-lab`**/**`screen-lab`**/**`v8-lab`**，以及 net-lab 4 个、eng-lab 5 个、render-lab 5 个场景（临时服务器用端口 0）。`eng-lab` 另有 `fixtures/`：故意写坏的示例业务目录（19 文件 / 33 条 import / 5 条违规），与实验台分离、随源码入库。
