...
## 结构
VitePress（`base:'/blog/'`，构建 ~55s），板块 `ai/ frontend/ interview/ node/`，配置 `.vitepress/config/*.js`，产物恒 `.vitepress/dist`。`node/` 11 模块、`frontend/基础/` 8 模块（含「工程协作」）、`frontend/进阶/` 13 模块（含「微前端体系」「可视化与图形」，模块清单与顺序以侧边栏配置为准），另散篇 `面试方法论.md` 与**改造方案文档 `frontend/2026-09-18-frontend进阶体系化改造方案.md`**（从 .workbuddy/plans 迁入，任务清单已标注完成状态）——都是**板块级独立入口**，不挂任何模块分组；frontend 全板块 148 篇含元页面（微前端体系 5 篇全落地、可视化与图形建到 2 篇）。**目录与文件名不带编号**，NN- 只用于 总结.md 分篇标题与「见第 X 篇」阅读序标签，侧边栏是唯一顺序来源。
...
**端口**：5174 HTML ·5175 CSS ·5176 JS ·5177 网络(+api 5178)·5179 工程化 ·5180 React·5181 Vue·5182 框架原理 ·5183 交付 ·5184 工程实践 ·5185 小程序 ·5186 TS·5187 性能 ·**5188+5189 都是 monitor-lab 的 `enterprise-server.js`**·5190 网络与协议进阶 ·**5191 渲染架构(render-lab)**。纯 CLI 探针不占端口：`esm-probe`、`mini-react`/`mini-vue`/**`mini-store`**/**`type-gym`**/**`runtime-lab`**/**`sandbox-lab`**/**`mf-demo`**/**`mini-micro`**/**`viz-lab`**，以及 net-lab 4 个、eng-lab 5 个、render-lab 5 个场景（临时服务器用端口 0）。`eng-lab` 另有 `fixtures/`：故意写坏的示例业务目录（19 文件 / 33 条 import / 5 条违规），与实验台分离、随源码入库。
